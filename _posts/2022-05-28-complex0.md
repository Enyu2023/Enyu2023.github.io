...
**证明**:任取开球$$B\subset G$$, 对于任何三角形围道$$T\in B$$, $$\int_T f_n=0\to\int_{T}f=0$$, 所以Morera定理给出$$f$$在$$B$$中解析. $$B$$是任意的，所以$$f\in H(G)$$.  
   取适当的紧集$$K$$和$$\delta$$, 使得$$\sup_K\vert g,h\vert <\delta\Rightarrow d(g,h)<\epsilon$$. 对于$$K$$, 可使用有限个球$$B^{r/2}_j, \bar B^{r}_j\subset G$$覆盖$$K$$,  现在，利用Cauchy估计, 对于$$z\in B^{r/2}$$:$$\begin{align*}\vert f_n^{(k)}(z)-f^{(k)}(z)\vert \leq&\frac{k!}{2\pi}\int_{\partial B^{r}}\left\vert \frac{f_n(w)-f(w)}{(w-z)^{k+1}}\right\vert \vert \mathrm d w\vert \\\leq& \frac{k!r}{(r/2)^{k+1}}\sup_K\vert f_n(w)-f(w)\vert \end{align*}$$
...
**证明:** $$\vert f\vert $$在$$\partial B$$上的最小值记为$$m>0$$,找到$$N$$ 当$$n>N$$时 $$\vert f_n-f\vert <\epsilon<m$$我们有
$$ \vert f_n-f\vert <\epsilon<\vert f\vert +\vert f_n\vert   $$
