...
$$\begin{align}    k=1,\cdots,n-1, \sup \vert f_n-f_k\vert \geq \epsilon\\\end{align}$$
...
$$\begin{align*}
    \vert f(z)-f(a)\vert \leq&\int_{\vert z-a\vert =2r} \left\vert \frac{f(w)(z-a)}{(w-z)(w-a)}\right\vert \vert \mathrm d w\vert \\\leq&\frac{2M}{r}\vert z-a\vert
\end{align*}$$
...
**step 2: $$\mathcal{F}$$非空，即至少存在把$$G$$映到$$\mathbb{D}$$内的共形映射:**
    ...
    **(c)**. 若$$\alpha=g_1(a),g_2=\phi_{\alpha}\circ g_1$$, $$g_2(a)=0$$. 再取
    $$\begin{align}
        g_3(z)=\frac{\vert g_2'(a)\vert }{g_2'(a)}g_2(z)\\
    \end{align}$$
...
**step 4:** 函数
    ...
    $$\begin{align}        h=\sqrt{\phi_w\circ f}, g=\frac{\vert h'(a)\vert }{h'(a)}\phi_{h(a)}\circ h\\\end{align}$$
    则$$g(G)\subset G, g(a)=0, g$$是单射, 且
    $$\begin{align}
        g'(a)=\frac{\vert h'(a)\vert }{h'(a)}\phi'_{h(a)}(h(a))h'(a)=\frac{1+\vert w\vert }{2\sqrt{w}}f'(a)>f'(a)\\
    \end{align}$$
